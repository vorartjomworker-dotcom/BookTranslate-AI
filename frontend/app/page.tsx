...
export default function Home() {
  ...
  async function loadBooks() {
    setLoading(true);
    try {
      const response = await fetch(`${apiUrl}/api/books`, { cache: "no-store" });
  ...
  async function uploadBook(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(null);
    try {
      const response = await fetch(`${apiUrl}/api/books/upload`, { method: "POST", body: data });
    ...
    <main className="shell library-shell">
      ...
      <section className="hero-grid">
        <div className="panel hero-copy">
          <span className="status-pill success">Stage 6</span>
          <h2>Translate, review, audit and export technical books without losing structure.</h2>
          <p className="muted">
            DOCX/EPUB ingestion, multi-model translation, human editing, book-level QA, terminology audit and translated DOCX/EPUB export are connected in one workflow.
          </p>
          <div className="metric-strip">
            <div><strong>{books.length}</strong><span>books</span></div>
            <div><strong>2</strong><span>source formats</span></div>
            <div><strong>2</strong><span>export formats</span></div>
          </div>
        </div>
        ...
        <form className="panel upload-panel" onSubmit={uploadBook}>
          <div className="panel-heading">
            <div>
              ...
              <h3>Import a technical book</h3>
            </div>
          </div>
          <label className="field">
            <span>DOCX or EPUB</span>
            <input type="file" name="file" accept=".docx,.epub" required />
          </label>
          <label className="field">
            <span>Display title (optional)</span>
            <input type="text" name="title" placeholder="Low-Latency C++" />
          </label>
          <div className="field-row">
            <label className="field">
              <span>Source</span>
              <input type="text" name="source_language" defaultValue="en" />
            </label>
            <label className="field">
              <span>Target</span>
              <input type="text" name="target_language" defaultValue="ru" />
            </label>
          </div>
      ...
      <section className="library-section">
        ...
        <div className="book-grid">
          {!loading && books.length === 0 ? (
            <div className="panel empty-state">Import your first DOCX or EPUB to start the translation workflow.</div>
          ) : null}
          {books.map((book) => (
            <Link className="book-card" href={`/books/${book.id}`} key={book.id}>
              ...
              <div className="book-meta">
                <span>{book.source_language.toUpperCase()} → {book.target_language.toUpperCase()}</span>
