...
def test_scheduler_skips_provider_in_feedback_cooldown() -> None:
    async def run() -> None:
        ...
        try:
            ...
            # The integration suite intentionally uses multiple asyncio.run() calls.
            # Redis connection objects retain their originating loop, so the test
            # that first opens the shared pool must close it before that loop exits.
