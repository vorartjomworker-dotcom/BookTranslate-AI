...
async def create_book_translation_job(
    book_id: uuid.UUID,
...
async def create_chapter_translation_job(
...
async def list_translation_jobs(book_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> list[TranslationJobResponse]:
    rows = list(
        (
            await db.execute(
                select(TranslationJob).where(TranslationJob.book_id == book_id).order_by(TranslationJob.created_at.desc())
            )
        ).scalars().all()
    )
